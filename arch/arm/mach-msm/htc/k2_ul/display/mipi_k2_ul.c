...
#include "mipi_k2_ul.h"

static struct mipi_dsi_panel_platform_data *mipi_k2_ul_pdata;

static int mipi_k2_ul_lcd_init(void);
...
static char dsi_k2_ul_pwm3[] = {0x55, 0x00};/* DTYPE_DCS_WRITE1 *///CABC off
...
static char dsi_k2_ul_pwm2[] = {0x53, 0x24};/* DTYPE_DCS_WRITE1 *///bkl on and no dim

static struct dsi_cmd_desc k2_ul_video_on_cmds[] = {
...
static struct dsi_cmd_desc k2_ul_color_enhance[] = {
...
static struct dsi_cmd_desc k2_ul_cmd_on_cmds[] = {
	...
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, sizeof(dsi_k2_ul_pwm2), dsi_k2_ul_pwm2},
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0, sizeof(dsi_k2_ul_pwm3), dsi_k2_ul_pwm3},
	{DTYPE_DCS_WRITE,  1, 0, 0, 120, sizeof(exit_sleep), exit_sleep},
};

static struct dsi_cmd_desc k2_ul_display_off_cmds[] = {
	{DTYPE_DCS_WRITE, 1, 0, 0, 10,
		sizeof(display_off), display_off},
	{DTYPE_DCS_WRITE, 1, 0, 0, 120,
		sizeof(enter_sleep), enter_sleep}
};
static struct dsi_cmd_desc k2_ul_display_on_cmds[] = {  //brandon, check the value
	{DTYPE_DCS_WRITE, 1, 0, 0, 0, sizeof(display_on), display_on},
};


static struct dsi_cmd_desc k2_ul_cmd_backlight_cmds[] = {
	{DTYPE_DCS_WRITE1, 1, 0, 0, 0,
		sizeof(led_pwm1), led_pwm1},
};

static int k2_ul_send_display_cmds(struct dsi_cmd_desc *cmd, int cnt)
{
	int ret = 0;
...
static int mipi_k2_ul_lcd_on(struct platform_device *pdev)
{
	...
	if (mfd->key != MFD_KEY)
        ...
        if (mipi_lcd_on && first_init)
          {
            k2_ul_send_display_cmds(k2_ul_color_enhance,
			ARRAY_SIZE(k2_ul_color_enhance));
	...
	mipi  = &mfd->panel_info.mipi;
        ...
        k2_ul_send_display_cmds(k2_ul_display_on_cmds, ARRAY_SIZE(k2_ul_display_on_cmds));
	if (mipi->mode == DSI_VIDEO_MODE)
          {
            k2_ul_send_display_cmds(k2_ul_video_on_cmds,
                                  ARRAY_SIZE(k2_ul_video_on_cmds));
          } 
        else 
          {
            k2_ul_send_display_cmds(k2_ul_cmd_on_cmds,
                                  ARRAY_SIZE(k2_ul_cmd_on_cmds));
          }
...
static int mipi_k2_ul_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	...
	if (mfd->key != MFD_KEY)
        ...
        if (panel_type != PANEL_ID_NONE)
          k2_ul_send_display_cmds(k2_ul_display_off_cmds,
                                ARRAY_SIZE(k2_ul_display_off_cmds));
...
static unsigned char k2_ul_shrink_pwm(int val)
{
...
static void mipi_k2_ul_set_backlight(struct msm_fb_data_type *mfd)
{
	struct mipi_panel_info *mipi;
///HTC:
        led_pwm1[1] = k2_ul_shrink_pwm(mfd->bl_level);
///:HTC

	if (mipi_k2_ul_pdata && (mipi_k2_ul_pdata->enable_wled_bl_ctrl)
	...
	k2_ul_send_display_cmds(k2_ul_cmd_backlight_cmds,
                              ARRAY_SIZE(k2_ul_cmd_backlight_cmds));
...
static int __devinit mipi_k2_ul_lcd_probe(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	struct mipi_panel_info *mipi;
	...
	if (pdev->id == 0) {
		mipi_k2_ul_pdata = pdev->dev.platform_data;

		if (mipi_k2_ul_pdata
			&& mipi_k2_ul_pdata->phy_ctrl_settings) {
			phy_settings = (mipi_k2_ul_pdata->phy_ctrl_settings);
		}

		if (mipi_k2_ul_pdata
			&& mipi_k2_ul_pdata->dlane_swap) {
			dlane_swap = (mipi_k2_ul_pdata->dlane_swap);
		}
...
static struct platform_driver this_driver = {
	.probe  = mipi_k2_ul_lcd_probe,
	.driver = {
		.name   = "mipi_k2_ul",
	},
};

static struct msm_fb_panel_data k2_ul_panel_data = {
	.on		= mipi_k2_ul_lcd_on,
	.off		= mipi_k2_ul_lcd_off,
	.set_backlight  = mipi_k2_ul_set_backlight,
};

static int ch_used[3];

int mipi_k2_ul_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	ret = mipi_k2_ul_lcd_init();
	if (ret) {
		pr_err("mipi_novatek_lcd_init() failed with ret %u\n", ret);
		return ret;
	}

	pdev = platform_device_alloc("mipi_k2_ul", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	k2_ul_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &k2_ul_panel_data,
		sizeof(k2_ul_panel_data));
	if (ret) {
		printk(KERN_ERR
...
//module_init(mipi_k2_ul_lcd_init);
